...
impl fmt::Display for SessResp {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self.action {
            ref grant @ SessReqAction::Grant => write!(
                f,
                "{} for {} seconds. {} renewals allowed.",
                grant, self.duration, self.renewals_remaining
            ),
            ref renew @ SessReqAction::Renew => write!(
                f,
                "{} for {} seconds. {} renewals remaining.",
                renew, self.duration, self.renewals_remaining
            ),
            ref too_soon @ SessReqAction::DenyRenewTooSoon => {
                write!(f, "{}, renewal ok in {} seconds.", too_soon, self.duration)
            }

            ref deny => write!(f, "{}", deny),
        }
    }
}
